...
export async function exportPptx(deck: Deck) {
  ...
  for (const slide of deck.slides) {
    ...
    for (const op of slideToOps(slide)) {
      if (op.kind === "rect") {
        ...
        s.addText(label(op), {
          ...
          y: px(op.y),
          w: px(op.w),
          h: px(op.size * 1.4 * Math.max(1, Math.ceil(op.text.length / Math.max(1, op.w / (op.size * 0.5))))),
          fontSize: op.size * 0.75,
          color: op.color,
          bold: op.bold ?? false,
          italic: op.italic ?? false,
          fontFace: op.font === "serif" ? "Georgia" : "Calibri",
          align: "left",
          valign: "top",
          margin: 0,
